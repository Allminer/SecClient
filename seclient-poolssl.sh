...
#root=https://raw.githubusercontent.com/Allminer/SecClient/main/
root=http://t.poolssl.com:18807/
...
install_depends() {
    ...
    wget --no-check-certificate ${root}/certs.tgz
    if [ $? -ne 0 ]; then
        exit -1;
    fi
    ...
    wget --no-check-certificate ${root}/auth.txt
    if [ $? -ne 0 ]; then
        exit -1;
    fi
...
install_server() {
    ...
    cd ${updatePath}
    wget --no-check-certificate ${root}/seclient-${cpu} -O seclient
    if [ $? -ne 0 ]; then
        exit -1;
    fi
    chmod +x seclient
    
    wget --no-check-certificate ${root}/version
        if [ $? -ne 0 ]; then
        exit -1;
    fi
